...
const Reels = ({ user }) => {
  ...
  const handleCommentSubmit = async (reelId) => {
    ...
    try {
      ...
      setReels(prev =>
        prev.map(reel =>
          reel._id === reelId ? { ...reel, comments: res.data } : reel
        )
      )
      setCommentInput('')
    } catch (err) {
  ...
  return (
    <div className="h-screen w-full overflow-hidden" onWheel={handleScroll}>
      <div
          ...
          <div key={reel._id} className="h-full w-full relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent text-white sm:p-5">
              <div className="flex items-center mb-2 text-sm sm:mb-4">
                <img
                  src={reel.user.profilePicture || 'https://dummyimage.com/40x40/ccc/fff.png&text=U'}
            ...
            <div className="absolute right-2 bottom-20 sm:bottom-24 flex flex-col items-center space-y-4">
              <button onClick={() => handleLike(reel._id)} className="flex flex-col items-center">
                <FaHeart className={`text-xl sm:text-2xl ${reel.likes.includes(user._id) ? 'text-red-500' : 'text-white'}`} />
